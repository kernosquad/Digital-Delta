...
export class SyncService {
  private async applyOperation(op: {
    ...
    await db.from(table).where('id', op.entity_id).update({ [op.field_name]: value })
  }
  ...
  async pull(ctx: HttpContext) {
    ...
    if (isFirstSync) {
      result.bootstrap = {
        ...
        sync_nodes: await db
          .from('sync_nodes')
          .select('id', 'node_uuid', 'public_key', 'is_relay'),
      }
    }
  ...
  async resolveConflict(ctx: HttpContext, payload: ResolveConflictType) {
    ...
    await db.from('sync_conflicts').where('id', params.id).update({
    ...
    await db
      .from('crdt_operations')
      .whereIn('id', [
        db.from('sync_conflicts').where('id', params.id).select('op_a_id'),
        db.from('sync_conflicts').where('id', params.id).select('op_b_id'),
      ])
