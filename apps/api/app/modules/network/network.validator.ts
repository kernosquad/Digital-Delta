...
export const storeEdgeValidator = vine.compile(
  vine.object({
    ...
    source_location_id: vine.number().min(0),
    target_location_id: vine.number().min(0),
    route_type: vine.enum(['road', 'river', 'airway'] as const),
    base_travel_mins: vine.number().min(0),
    max_payload_kg: vine.number().optional(),
    allowed_vehicles: vine.array(vine.enum(['truck', 'speedboat', 'drone'] as const)).optional(),
  })
);
...
export const edgeStatusValidator = vine.compile(
  vine.object({
    ...
    current_travel_mins: vine.number().min(0).optional(),
    risk_score: vine.number().min(0).max(1).optional(),
    reason: vine
      .enum(['flood', 'recede', 'ml_prediction', 'manual_override', 'chaos_engine'] as const)
      .optional(),
  })
);
